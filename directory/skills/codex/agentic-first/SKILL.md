...
description: >-
  ...
  https://agentic-first.co/directory/standard/). Use when the user wants to
  ...
  domain to the directory at agentic-first.co/directory. v0.1.4 revises
  ...
  *.gamma.site, Tome, Beautiful.AI) from a multi-line block to a single
  ...
  github.com/yqup/agentic-first/tree/main/directory/docs/recipes — fetch only the
...
<https://agentic-first.co/directory/standard/>. Live directory MCP at
...
>   `https://agentic-first.co/directory/schemas/` rather than hand-coding
...
2. **Pick the right schema before you start.** Four schemas, two
   dimensions:
    ...
    |---|---|---|
    | `company` | `public` | `https://agentic-first.co/directory/schemas/company-profile-0.2.0.json` |
    | `company` | `protected` | `https://agentic-first.co/directory/schemas/company-private-profile-0.1.0.json` |
    | `person` | `public` | `https://agentic-first.co/directory/schemas/personal-profile-0.1.0.json` |
    | `person` | `protected` | `https://agentic-first.co/directory/schemas/personal-private-profile-0.1.0.json` |
...
For **company / public**: <https://agentic-first.co/directory/standard/#company-public>.
For **personal / public**: <https://agentic-first.co/directory/standard/#person-public>.
For **company / protected**: <https://agentic-first.co/directory/standard/#company-protected>.
For **personal / protected**: <https://agentic-first.co/directory/standard/#person-protected>.
...
- `schema_version: "0.2.0"` for company/public profiles, or
  `"0.1.0"` for company/protected, person/public, and person/protected
  profiles.
...
- [ ] `schema_version` matches the selected schema (`"0.2.0"` for
      company/public, `"0.1.0"` for the other current schemas).
...
4. **AI-builder hosts that have NO HTML primitive at all (Mode 5,
   revised in v0.1.4)** (Gamma free / `*.gamma.site`, Tome,
   Beautiful.AI — anywhere the only thing the user can put on a page
   is text): tell the user to paste a **single line** of text into a
   footer card. Wire format is one line:
   ...
   Empirically these *backfire* on AI-builder hosts because the
   publish AI reads them as instructions to itself. The colophon's
   ...
   <https://github.com/yqup/agentic-first/blob/main/directory/docs/recipes/modes/05-plaintext-block.md>.
   Soft warning (lowest trust of the five modes — host AI may
   paraphrase values), but it's the only path that works on
   text-only hosts. (v0.1.3 specified a multi-line block + "Do not
   edit" preamble; v0.1.4 retracts both same-day after live testing on
...
5. **If the host isn't in the table**, fetch
   ...
   and check the "Hosts not in the table" section. The universal
   escape hatch is to front the user's domain with **any** static
   host that does path-based rewrites/proxies — Vercel
   (`vercel.json`), Netlify (`_redirects`), Cloudflare Pages
   (`_routes.json`), Cloudflare Workers, Bunny.net, GitHub Pages on a
   custom domain, or any self-hosted reverse proxy. The standard does
   not require any specific vendor; the worker / `vercel.json` /
   `_redirects` form is just an implementation detail. Pick whichever
   ergonomics the user already has set up. Alternative: publish on a
   separate static-host subdomain.
...
6. **Verify locally** before handing off:
   ...
   For mode 5, swap to
   `curl -sSL https://{their-domain}/ | sed -e 's/<[^>]*>//g' | grep -F 'agentic-first profile v0.1.0'`
   — strips HTML and matches the colophon marker in the rendered text.

The full recipe tree (modes + hosts) browseable at
<https://github.com/yqup/agentic-first/tree/main/directory/docs/recipes>.
...
Full directory-side ruleset: <https://agentic-first.co/directory/security/>.
