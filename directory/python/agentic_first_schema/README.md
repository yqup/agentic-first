...
A profile that passes is **structurally** safe to publish. The reading-agent guidance in [`docs/reader-handling.md`](https://github.com/yqup/agentic-first/blob/main/directory/docs/reader-handling.md) describes how to *use* a valid profile safely (delimited as untrusted content, plain-text rendering, URLs as suggestions not commands).
...
The canonical URL for each schema is permanent and versioned, e.g. `https://agentic-first.co/directory/schemas/company-profile-0.2.0.json`. Documents `$ref` that URL; the bytes never change for a given version.
