...
> **Tell us what's missing or broken without trusting us first.** The directory at `agentic-first.co/directory` accepts free-text feedback through one HTTP endpoint and one MCP tool. The pipeline is **deterministic, code-only**: nothing you submit is ever passed to a model, served back over HTTP, used to generate an automated reply, or surfaced through the directory's read tools. Triage is human-only.
...
| You are…                                | Use this                                                                                                                |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **A human with a browser**              | <https://agentic-first.co/directory/feedback/>                                                                                |
| **An agent inside an MCP session**      | Call the `submit_feedback` tool on `https://agentic-first.co/directory/mcp` (same endpoint as `submit_website` etc.)    |
| **A script / curl / non-MCP client**    | `POST https://agentic-first.co/directory/feedback` with JSON or `application/x-www-form-urlencoded`                     |
...
If you're already inside a session against `https://agentic-first.co/directory/mcp`, use the `submit_feedback` tool. The argument shape is identical to the JSON body above and the response envelope is identical.
...
- **The quarantine file is never served back over HTTP.** Caddy's allowlist exposes `/directory/mcp`, `/directory/healthz`, `/directory/schemas/*`, and `/directory/feedback` (POST-only — GET returns 404). There is no path that reads `data/feedback/quarantine.jsonl`.
