...
function nodeServerFor(site) {
  ...
  if (mcpLoopbackFor(site)) {
    ...
    const configuredMcpTargets = Array.isArray(site.mcp?.container_proxy_urls)
      ? site.mcp.container_proxy_urls
      : site.mcp?.container_proxy_url
        ? [site.mcp.container_proxy_url]
        : [defaultMcpProxyTarget];
    serverConfig.mcpProxy = {
      path: publicMcpPathFor(site),
      target: configuredMcpTargets[0],
      targets: configuredMcpTargets,
    };
  }
  if (site.briefing?.mailerlite) {
